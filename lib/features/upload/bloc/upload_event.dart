...
class UploadEvent with _$UploadEvent {
  ...
  const factory UploadEvent.pauseUpload({String? fileId}) = PauseUpload;

  /// Resume upload process
  ///
  /// [fileId] - Specific file to resume (optional, resumes all if null)
  const factory UploadEvent.resumeUpload({String? fileId}) = ResumeUpload;

  /// Cancel upload process
  ///
  /// [fileId] - Specific file to cancel (optional, cancels all if null)
  const factory UploadEvent.cancelUpload({String? fileId}) = CancelUpload;
  ...
  const factory UploadEvent.removeFile({required String fileId}) = RemoveFile;
  ...
  const factory UploadEvent.validateFiles({required List<XFile> files}) =
      ValidateFiles;
  ...
  const factory UploadEvent.setCategory({required String categoryId}) =
      SetCategory;
  ...
  const factory UploadEvent.uploadError({required String error}) = UploadError;

  /// Handle overall progress update (internal event)
  ///
  /// [progress] - Overall progress (0.0 to 1.0)
  const factory UploadEvent.overallProgressUpdated({required double progress}) =
      OverallProgressUpdated;

  /// Handle file progress update (internal event)
  ///
  /// [fileId] - File ID
  /// [progress] - File progress (0.0 to 1.0)
  const factory UploadEvent.fileProgressUpdated({
    required String fileId,
    required double progress,
  }) = FileProgressUpdated;

  /// Handle file status update (internal event)
  ///
  /// [fileId] - File ID
  /// [status] - New status
  const factory UploadEvent.fileStatusUpdated({
    required String fileId,
