...
class FilenameUtils {
  ...
  static String getDisplayFileName(String storageFileName) {
    ...
    // Check if filename has timestamp prefix pattern (digits_filename)
  ...
  static String formatForDisplay(String fileName, {int? maxLength}) {
    ...
    if (maxLength != null && cleanName.length > maxLength) {
      ...
      if (extension.isNotEmpty) {
        final maxNameLength = maxLength - extension.length - 4; // Account for "..." and "."
        if (maxNameLength > 0) {
  ...
  static String createUniqueFileName(String fileName, List<String> existingNames) {
    final cleanName = getDisplayFileName(fileName);
