...
enum AddFilesFilterMode {
  modal, // Modal bottom sheet (default)
  embedded, // Embedded in page
}

class AddFilesFilterWidget extends StatelessWidget {
  final VoidCallback? onFilterApplied;
  final VoidCallback? onClose;
  final AddFilesFilterMode mode;

  const AddFilesFilterWidget({
    super.key,
    this.onFilterApplied,
    this.onClose,
    this.mode = AddFilesFilterMode.modal,
  });

  /// Factory constructor for modal filter (bottom sheet)
  factory AddFilesFilterWidget.modal({VoidCallback? onFilterApplied}) {
    return AddFilesFilterWidget(
      onFilterApplied: onFilterApplied,
      mode: AddFilesFilterMode.modal,
    );
  }

  /// Factory constructor for embedded filter (in page)
  factory AddFilesFilterWidget.embedded({
    ...
    return AddFilesFilterWidget(
      onFilterApplied: onFilterApplied,
      onClose: onClose,
      mode: AddFilesFilterMode.embedded,
    );
  }

  @override
  Widget build(BuildContext context) {
    return Consumer<AddFilesFilterState>(
      builder: (context, filterState, child) {
        return Container(
          padding: const EdgeInsets.all(20),
          decoration: BoxDecoration(
            color: AppColors.surface,
            borderRadius: mode == AddFilesFilterMode.modal
          ...
          child: Column(
            ...
            children: [
              // Header
              if (mode == AddFilesFilterMode.embedded) ...[
                Row(
              ...
              SizedBox(
                ...
                child: OutlinedButton.icon(
                  onPressed: () {
                    ...
                    if (mode == AddFilesFilterMode.modal) {
  ...
  Widget _buildFileTypeFilters(
    ...
    AddFilesFilterState filterState,
  ) {
