...
interface MetadataExtractionData {
  filePath: string;
  contentType: string;
}
...
async function calculateFileHash(filePath: string): Promise<string> {
  try {
    ...
    throw new functions.https.HttpsError(
      "internal",
      `Failed to calculate file hash: ${error}`
    );
  }
}
...
async function checkForDuplicates(
  ...
  try {
    ...
    if (!nameQuery.empty) {
      ...
      console.log(
        `Potential duplicate file detected by name and size: ${fileName}`
      );
      return {
        isDuplicate: true,
        existingDocument: existingDoc,
 ...
 */
const processFileUpload = functions.https.onCall(
  async (data: FileUploadData, context) => {
    ...
    try {
      ...
      const documentData = {
        ...
        metadata: {
          ...
          storageFileName: filePath.split("/").pop() || sanitizedFileName, // Actual storage filename with timestamp
      ...
      throw new functions.https.HttpsError(
        "internal",
        `Failed to process file upload: ${error}`
      );
    }
  }
);
 ...
 */
const validateFile = functions.https.onCall(
  async (data: FileValidationData, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
        "unauthenticated",
        "User must be authenticated"
 ...
 */
const extractMetadata = functions.https.onCall(
  async (data: MetadataExtractionData, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
        "unauthenticated",
        "User must be authenticated"
...
async function validateFileInternal(data: FileValidationData) {
...
async function extractMetadataInternal(data: MetadataExtractionData) {
...
const getStorageQuota = functions.https.onCall(async (_data, context) => {
  ...
  try {
    ...
    throw new functions.https.HttpsError(
      "internal",
      `Failed to get storage quota: ${error}`
    );
  }
});
 ...
 */
async function searchFileInStorage(fileName: string): Promise<string | null> {
  try {
 ...
 */
const getFileAccessUrl = functions.https.onCall(
  async (data: { filePath: string; expirationMinutes?: number }, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
        "unauthenticated",
        "User must be authenticated"
    ...
    try {
      ...
      if (!filePath || typeof filePath !== "string" || filePath.trim() === "") {
        ...
        throw new functions.https.HttpsError(
          "invalid-argument",
          "A valid file path must be specified"
        );
      }
      ...
      throw new functions.https.HttpsError(
        "internal",
        `Failed to generate file access URL: ${error}`
      );
    }
  }
);
...
const cleanupOrphanedFiles = functions.https.onCall(async (_data, context) => {
  ...
  try {
    ...
    throw new functions.https.HttpsError(
      "internal",
      `Failed to cleanup orphaned files: ${error}`
    );
  }
});
 ...
 */
const checkDuplicateFile = functions.https.onCall(
  async (
    data: {
      fileName: string;
      fileSize: number;
    ...
    try {
      ...
      throw new functions.https.HttpsError(
        "internal",
        `Failed to check for duplicates: ${error}`
      );
    }
  }
);
 ...
 */
const batchProcessFiles = functions.https.onCall(
  async (
    data: { filePaths: string[]; operation: string; options?: any },
    context
  ) => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
    ...
    try {
      ...
      throw new functions.https.HttpsError(
        "internal",
        `Failed to batch process files: ${error}`
      );
    }
  }
);
